...
if (!nexacro.Radio) {
	...
	_pRadio._on_radio_onkeydown = function (obj, e) {
		...
		if (keycode == nexacro.Event.KEY_TAB) {
			...
			var ret = this.on_fire_canitemchange(obj, pre_index, pre_text, pre_value, idx, radioitem.text, radioitem.value);
			if (ret) {
				this._accessibility_index = idx;
				if (row_cnt != idx && row_cnt >= idx && 0 <= idx) {
					this.set_index(idx);
					if (idx != pre_index) {
						this.on_fire_onitemchanged(obj, pre_index, pre_text, pre_value, idx, radioitem.text, radioitem.value);
					}
				}
			...
			if (ds && item_len) {
				...
				if (radio_columncount == -1 && radio_rowcount == -1) {
					...
					if (dir == "horizontal") {
						...
						if (radio_rowcount > 0 && ds_rowcount >= radio_rowcount) {
							apply_rowcnt = radio_rowcount;
						}
						else if (radio_columncount > 0) {
							apply_rowcnt = parseInt(ds_rowcount / radio_columncount);
